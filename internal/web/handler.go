...
func New(s *store.Store, users *auth.UserStore, tmplGlob string) (*Handler, error) {
	tmpl, err := template.New("").Funcs(template.FuncMap{
		...
		"statusIcon": func(s *engine.StepState) string {
			switch s.Status {
			case engine.StatusDone, engine.StatusEnded:	return "✓"
			case engine.StatusSkipped:                  return "–"
			case engine.StatusReady:                    return "▶"
			case engine.StatusAsk:                      return "?"
			case engine.StatusGate:                     return "🔑"
			default:                                    return "○"
			}
		},
		...
		"visibleNeeds": func(needs []string) string {
			var out []string
			for _, n := range needs {
				if n != "__ask_target__" { out = append(out, n) }
			}
		...
		"roleLabel": func(r auth.Role) string {
			switch r {
			case auth.RoleAdmin:  return "Admin"
			case auth.RoleUser:   return "User"
			case auth.RoleViewer: return "Viewer"
			}
			return string(r)
		},
		"isAdmin":  func(u *auth.User) bool { return u != nil && u.CanAdmin() },
		"initial": func(s string) string { if len(s)==0 { return "?" }; r:=[]rune(s); return string(r[0]) },
...
type column struct{ Title string; Cards []card }
...
func (h *Handler) board(w http.ResponseWriter, r *http.Request) {
	...
	for i, l := range filterLabels { filterLabels[i] = strings.ToLower(l) }
	...
	for _, inst := range instances {
		if q != "" && !strings.Contains(strings.ToLower(inst.Title+" "+inst.WorkflowName), q) { continue }
		if len(filterPriorities) > 0 && !containsStr(filterPriorities, inst.Priority) { continue }
		if len(filterLabels) > 0 && !hasAnyLabel(inst.Labels, filterLabels) { continue }
		if filterDue != "" && !matchDueFilter(inst, filterDue, now) { continue }
		if filterCreated != "" && !matchCreatedFilter(inst, filterCreated, now) { continue }
		...
		switch {
		case string(inst.Status) == "done": cols["Done"].Cards = append(cols["Done"].Cards, c)
		case done == 0:                     cols["Todo"].Cards = append(cols["Todo"].Cards, c)
		default:                            cols["In Progress"].Cards = append(cols["In Progress"].Cards, c)
		}
	}
...
func (h *Handler) advanceStep(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.store.AdvanceStep(id, r.FormValue("step")); err != nil {
		flashError(w, r, err.Error()); return
	}
	http.Redirect(w, r, "/instance/"+id, http.StatusSeeOther)
}
...
func (h *Handler) approvalPage(w http.ResponseWriter, r *http.Request) {
	...
	if inst == nil || step == nil {
		h.render(w, r, "approve.html", approvalData{Token: token, Error: "Link nicht gefunden oder bereits verwendet."}); return
	}
	if step.GateUsed {
		h.render(w, r, "approve.html", approvalData{Token: token, Done: true, Step: step, Instance: inst}); return
	}
...
func (h *Handler) approvalSubmit(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.render(w, r, "approve.html", approvalData{Token: token, Error: err.Error()}); return
	}
...
func (h *Handler) adminDeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	if cu != nil && cu.ID == id {
		...
		http.Redirect(w, r, "/admin/users", http.StatusSeeOther); return
	}
	h.users.Delete(id)
	http.Redirect(w, r, "/admin/users", http.StatusSeeOther)
}
...
func hasAnyLabel(labels []string, filters []string) bool {
	for _, f := range filters {
		for _, l := range labels { if strings.ToLower(l) == f { return true } }
	}
	return false
}
...
func matchDueFilter(inst *engine.Instance, filter string, now time.Time) bool {
	switch filter {
	case "overdue": return inst.HasOverdue()
	case "today":
		end := time.Date(now.Year(), now.Month(), now.Day(), 23, 59, 59, 0, now.Location())
		var found bool
		inst.AllStepsDue(func(due time.Time) { if !due.After(end) && due.After(now) { found = true } })
		return found
	case "7d":
		end := now.Add(7 * 24 * time.Hour)
		var found bool
		inst.AllStepsDue(func(due time.Time) { if due.After(now) && due.Before(end) { found = true } })
		return found
	}
	return true
}
...
func matchCreatedFilter(inst *engine.Instance, filter string, now time.Time) bool {
	...
	case "7d":  return inst.CreatedAt.After(now.Add(-7 * 24 * time.Hour))
	case "30d": return inst.CreatedAt.After(now.Add(-30 * 24 * time.Hour))
	}
	return true
}
...
func hasLabel(labels []string, filter string) bool {
	for _, l := range labels { if strings.ToLower(l) == filter { return true } }
	return false
}
